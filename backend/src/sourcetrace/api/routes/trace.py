...
from sourcetrace.retrieval.trace import FlowTraceResult, FlowTraceService
...
class FlowTraceRequest(BaseModel):
    ...
    mode: Literal["static"] = Field(
        default="static", description="Trace mode; only zero-token static is available"
    )
...
def _result_to_response(repository_id: str, result: FlowTraceResult) -> FlowTraceResponse:
