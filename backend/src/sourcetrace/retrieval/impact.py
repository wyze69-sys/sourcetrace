...
class ChangeImpactService:
    ...
    def preview(
        ...
        upstream = self._bfs(
            start_id=target_chunk.chunk_id,
        ...
        for node_id in sorted(
            involved_ids, key=lambda i: chunk_sort_key(by_id[i])
        ):
    ...
    def _bfs(
        ...
        A node's confidence is the weakest edge confidence along its discovery
        path; when several frontier edges reach the same new node, the highest
        combined confidence wins and earlier edges (fixed build order) break
        ties. Frontier nodes are expanded in chunk sort order.
        """
        ...
        frontier: dict[str, int] = {start_id: _CONFIDENCE_RANK["high"]}
        depth = 0
    ...
    def _affected_endpoints(
        ...
        involved = [target_chunk] + [by_id[item.node_id] for item in upstream]
